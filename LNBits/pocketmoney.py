...
def create_invoice(wallet_id, invoice_key, amount_eur, child_name):
    ...
    try:
        ...
        logger.error(f"Error creating invoice for {child_name} with wallet {wallet_id}: {e}")
        bot.send_message(CHAT_ID, f"Error creating invoice for {child_name} with wallet {wallet_id}: {e}")
        return None, None
...
def scheduled_transfer():
    for child, wallet_info in CHILDREN_WALLETS.items():
        ...
        payment_request, amount_sats = create_invoice(wallet_id, invoice_key, fiat_amount, child)
        if payment_request:
            pay_invoice(payment_request)
            logger.info(f"Transferred {fiat_amount} EUR ({amount_sats} satoshis) worth of pocket money to {child}")
...
def schedule_transfers():
    if RECURRENCE == 'daily':
        ...
        try:
            ...
            raise ValueError("Invalid day of week. Choose from 'monday', 'tuesday', etc.")
    else:
        raise ValueError("Invalid recurrence setting. Choose from 'daily' or 'weekly'.")
...
def main():
    ...
    parser.add_argument('--run-once', action='store_true', help="Run the transfer once and exit, best for cronjobs")
