...
lndg_api_url = (
    config["lndg"]["lndg_api_url"]
    + "/api/channels/?is_active=true&is_open=true&limit=300&offset=0"
)
...
def convert_short_to_long_chan_id(short_chan_ids):  # Now accepts a list
    ...
    payload = {"query": bulk_query, "variables": variables}
...
def cluster_sold_channels():
    ...
    for short_chan_id, info in short_id_info.items():
        ...
        if not long_chan_id:
            logging.error(
                f"Warning: No long channel ID found for short channel ID {short_chan_id}"
            )
            continue
        ...
        if status == "VALID_CHANNEL_OPENING":
            ...
            fee_grace_calculation = sum(
                [-1 * (min_block_length - blocks_until_close - fee_grace_period)]
            )
            active_channels_info.append(
                (long_chan_id, blocks_until_close, fee_cap, fee_grace_calculation)
            )
            ...
            logging.info(
                f"Channel {long_chan_id} with status {status} and blocks until close {blocks_until_close} is not processed."
            )

    # Write channel IDs to their respective files
    for fee_cap, channel_ids in fee_cap_groups.items():
        file_path = get_fee_cap_file_path(
            fee_cap
        )  # Function to generate file path based on fee cap
...
def update_autofees(non_active_chan_ids):
    ...
    def fetch_current_channel_states():
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logging.error(
                    f"{timestamp} Failed to fetch current channel states, status code: {response.status_code}"
                )
        except Exception as e:
    ...
    channels_to_update = [
        chan_id for chan_id in non_active_chan_ids if chan_id in current_channel_states
    ]
        ...
        try:
            ...
            if response.status_code == 200:
                with open(log_file_path, "a") as log_file:
                    log_file.write(
                        f"{timestamp}: Updated auto_fees for channel {chan_id}\n"
                    )
                logging.info(f"Updated auto_fees for channel {chan_id}")
            else:
                logging.error(
                    f"{timestamp}: Failed to update auto_fees for channel {chan_id}: Status Code {response.status_code}"
                )
    ...
    for item in active_channels_info:
        ...
        try:
            ...
            if response.status_code == 200:
                with open(log_file_path, "a") as log_file:
                    log_file.write(
                        f"{timestamp}: Updated notes for channel {chan_id}\n"
                    )
                logging.debug(f"Updated notes for channel {chan_id}")
            else:
                logging.error(
                    f"{timestamp}: Failed to update notes for channel {chan_id}: Status Code {response.status_code}"
                )
