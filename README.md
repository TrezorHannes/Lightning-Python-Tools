...
| [`Other/swap_out-loop.py`](Other/swap_out-loop.py) | `CLI tool` | Economically optimized Loop Out liquidity rebalancing via Lightning Labs Loop (`litloop` / `loopd`) with live route probing, multi-channel outbound route selection (MPP diversity), economic scoring, interactive UI, and SQLite accounting. See [documentation](Other/swap_out-loop.md). |
